...
const Algorithm = ({ title, prompt, soln, bigO }: Algo) => {
  ...
  return (
    <article className="mx-auto w-full max-w-7xl p-2 sm:p-20">
      <h2 className="text-6xl text-slate-200">{title}:</h2>
      <p className="py-10 indent-16 text-xl leading-8 text-gray-400">
        {prompt}
      </p>
      <div className="relative w-full max-w-full rounded-md border border-dashed p-10">
        ...
        <pre className="overflow-x-auto py-5">
          <CodeBlock soln={soln} />
        </pre>
      </div>

      <h2 className="pt-20 text-6xl text-slate-200">Big O:</h2>
      <div className="overflow-x-auto p-10 text-xl leading-8 text-gray-400">
        <pre>{bigO}</pre>
      </div>
    </article>
  );
